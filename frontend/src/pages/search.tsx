...
interface porductSearchProps {}

const porductSearch: React.FC<porductSearchProps> = ({}) => {
  ...
  return (
    <div>
      ...
      >
        <div className="flex">
          <input
            ...
            className="h-16 px-2 text-white w-full bg-gray-700 rounded-none focus:border-gray-700"
          ></input>
          <button
      ...
      <div className="p-2">{listening && <p>Listening...</p>}</div>
      <div className="p-2">{result && <p>Result: {result}</p>}</div>
    </div>
  );
};

export default porductSearch;
